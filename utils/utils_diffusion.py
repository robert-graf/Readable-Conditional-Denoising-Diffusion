...
class Diffusions_Constance(nn.Module):
    def __init__(self, scheduler_name: str, timesteps: int = 1000, accuracy=torch.float32):
        ...
        self.p2_loss_weight_gamma = 0.0  # p2 loss weight, from https://arxiv.org/abs/2204.00227 - 0 is equivalent to weight of 1 across time - 1. is recommended
    ...
    def compute_constants(self, betas):
        self.s("betas", betas)
        ...
        self.s("sqrt_a_bar", torch.sqrt(a_bar))
        self.s("sqrt_one_minus_a_bar", torch.sqrt(1.0 - a_bar))
        self.s("sqrt_recip_a_bar", torch.sqrt(1.0 / a_bar))
        self.s("sqrt_recipm1_a_bar", torch.sqrt(1.0 / a_bar - 1))
        # calculations for posterior q(x_{t-1} | x_t, x_0)
        posterior_variance = betas * (1.0 - a_bar_prev) / (1.0 - a_bar)
        self.s("posterior_variance", posterior_variance)
        # below: log calculation clipped because the posterior variance is 0 at the beginning of the diffusion chain
        # this is min variance
        self.s("posterior_log_variance_clipped", torch.log(posterior_variance.clamp(min=1e-20)))
        self.s("log_variance", torch.log(betas))
        self.s("posterior_mean_coef1", betas * torch.sqrt(a_bar_prev) / (1.0 - a_bar))
        self.s("posterior_mean_coef2", (1.0 - a_bar_prev) * torch.sqrt(alphas) / (1.0 - a_bar))
        # scales the loss in the training step. Paper said it can be ignored.
        self.s(
            "p2_loss_weight",
            ((self.p2_loss_weight_k + a_bar / (1 - a_bar)) ** -self.p2_loss_weight_gamma),
        )

    def backward_step_to_x0(self, t, shape):
        c1 = extract(self.sqrt_recip_a_bar, t, shape)  # type: ignore
        c2 = extract(self.sqrt_recipm1_a_bar, t, shape)  # type: ignore
        return c1, c2

    def mean_step_to_x_t_new(self, t, shape) -> Tuple[Tensor, Tensor]:
        # coef1 = betas * torch.sqrt(a_bar_prev) / (1. - a_bar)
        c1 = extract(self.posterior_mean_coef1, t, shape)  # type: ignore
        # coef2 = (1. - a_bar_prev) * torch.sqrt(a) / (1. - a_bar)
        c2 = extract(self.posterior_mean_coef2, t, shape)  # type: ignore
        return c1, c2
    ...
    def compute_new_mean(self, e, t, x_t, noise_mode=True, clamping=None) -> Tensor:
        ...
        # Equation 7 from the paper
        posterior_mean = self.compute_posterior_mean(x_0, x_t, t)
    ...
    def ddim_alphas(self, t: Tensor, t_next: Tensor) -> Tuple[Tensor, Tensor]:
        beta: Tensor = torch.cat([torch.zeros(1).to(self.betas.device), self.betas], dim=0).to(self.accuracy)  # type: ignore
    ...
    def weight_and_mean_loss(self, loss, t):
