...
def setup_reg_affine(start_path: Path, key: str, sequ: str, axis: int, filter=None):
    ...
    for p in Path(start_path, "registration", key, folder).iterdir():
        if p.name.endswith("dixon.nii.gz"):
            ...
            seg = p
    #
    ct_arr = to_nii(ct).reorient_(("R", "I", "P"))
    mi_arr = to_nii(dixon).reorient_(("R", "I", "P"))
    sg_arr = to_nii(seg).reorient_(("R", "I", "P"))
...
def __main(png, j):
    for search_path in ["/media/data/robert/datasets/spinegan_T2w_2p_reg/"]:
        ...
        try:
            ...
            if png:
                out_path = "/media/data/robert/datasets/spinegan_T2w_2p_reg/img/deform"
            elif j == 10:
                out_path = f"/media/data/robert/datasets/spinegan_T2w_2p_reg/val/"
            elif j == 11:
                out_path = f"/media/data/robert/datasets/spinegan_T2w_2p_reg/test/"
            else:
                out_path = f"/media/data/robert/datasets/spinegan_T2w_2p_reg/train/{j}/"
...
def generate_file_path_reg(out, file: BIDS_FILE, other_file_id, folder, keys=["sub", "ses", "sequ", "reg", "acq"]):
    info = file.info.copy()
    ...
    out_file: Path = file.get_changed_path(
        dataset_path=f"/media/data/robert/datasets/{out}/",
        file_type="nii.gz",
        parent="registration",
        path="{sub}/" + folder,
        info=info,
...
def __register(subj_name, subject: Subject_Container):
    ...
    for bids_file in query.loop_dict():
        ...
        msk = bids_file["msk"][0]
        procspin = bids_file["msk_procspin"][0] if "msk_procspin" in bids_file else None
        e = msk.get("e")
        ...
        for bids_file in query.loop_dict():
            ...
            ctd_mr.centroids = {k + Location.Vertebra_Corpus.value * 100: v for k, v in ctd_mr.centroids.items()}
            if procspin is not None:
                ctd_procspin = calc_centroids_labeled_buffered(msk_reference=procspin, subreg_reference=None)
                for k, v in ctd_procspin.items():
                    ctd_mr[k + Location.Spinosus_Process.value * 100] = v

            ctd_ct = calc_centroids_labeled_buffered(
                msk_reference=vert,
                subreg_reference=subreg,
                subreg_id=[Location.Vertebra_Corpus.value, Location.Spinosus_Process.value],
            )
            ...
            reg_segmentation.ridged_registration_by_dict_from_ctd(
                ...
                ctd_mr,
                ctd_ct,
                generate_file_names=partial(generate_file_path_reg, "spinegan_T2w_2p_reg"),
                axcodes_to=("R", "I", "P"),
                voxel_spacing=(-1, 1, 1),
                snap_shot_folder="/media/data/robert/datasets/spinegan_T2w_2p_reg/",
            )
