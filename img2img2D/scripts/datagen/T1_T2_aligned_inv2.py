...
out_path = Path("/media/data/robert/datasets/nako_upscale_t1_t2_inv/")
...
def __help(idx, sub, region_t1: dict[str, BIDS_FILE], region_t2: dict[str, BIDS_FILE], max_img):
    ...
    for key, val in region_t1.items():
        ...
        for s in ["HWS", "BWS", "LWS"]:
            moving = NII.load_bids(region_t2[s])
            ...
            if intersect_z(moving, fixed):
                ...
                slice_and_save(
                    f_out, m_out, other_moving[0], other_moving[1], phase, str(idx) + "_" + s + f"_{length:.3f}"
                )
