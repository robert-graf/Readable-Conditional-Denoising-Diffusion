...
class ColorJitterSphere3D:
    ...
    def __init__(
        self, brightness_min_max: tuple = None, contrast_min_max: tuple = None, sigma: float = 1.0, dims: int = 3
    ) -> None:
        self.brightness_min_max = brightness_min_max
        self.contrast_min_max = contrast_min_max
        self.sigma = sigma
...
class RandomRotate:
    ...
    def rotate(self, input_tensor: torch.Tensor, rotation_matrix: torch.Tensor) -> torch.Tensor:
        ...
        rotated_3d_positions = torch.bmm(rotation_matrix.view(1, 3, 3).expand(d * h * w, 3, 3), locations_3d).view(
            1, d, h, w, 3
        )
        ...
        grid = (
            torch.stack([normalized_locs_x, normalized_locs_y, normalized_locs_z], dim=4)
            .view(1, d, h, w, 3)
...
class RandomScale:
    ...
    def __call__(self, x: torch.Tensor) -> torch.Tensor:
        ...
        for i, d in enumerate(dims):
            locations.append(
                torch.linspace(-1, 1, d, device=device_).view(*[1] * (i + 1), d, *[1] * (dim - i - 1)).expand(1, *dims)
            )
...
class RandomNoise:
    ...
    def __init__(
        self, mean: float = 0, std: Tuple[float, float] = (0, 0.25),
    ):
        self.mean = mean
        self.std_range = std
...
class RandomBiasField:
    ...
    *Automated model-based tissue classification of MR images of the brain*.
    ...
    @staticmethod
    def generate_bias_field(data: torch.Tensor, order: int, coefficients: List[float],) -> np.ndarray:
